    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          user_input: {
            type: 'string',
            description: 'User input to analyze for promotion patterns',
          },
          thread_id: { type: 'string', description: 'Thread identifier (optional)' },
        },
        required: ['user_input'],
      ...
      case 'check_permission': {
        ...
        if (roleManager.isFridayAfternoon() && (action as string)?.includes('deploy')) {
          await roleManager.auditLog('permission_denied', {
            ...
            action,
            resource,
            reason: 'friday_afternoon_restriction',
          })
        ...
        if ((resource as string)?.startsWith?.('mcp_')) {
          ...
          allowed =
            capabilities.mcp_servers.includes('*') || capabilities.mcp_servers.includes(serverName)
        }
        // Check tool access
        ...
        // Check special permissions
          ...
          (action as string) &&
          capabilities.special_permissions.includes(action as string)
        ) {
          allowed = true
        }
          ...
          content: [
            {
              type: 'text',
              text: JSON.stringify(
                { promoted: false, reason: 'No promotion patterns matched' },
                null,
                2
              ),
            },
              ...
              text: JSON.stringify(
                {
                  ...
                  cost_per_operation:
                    state.operations_count > 0
                      ? `$${(estimatedCost / state.operations_count).toFixed(4)}`
                      : '$0.0000',
                  recommendations,
                },
                null,
