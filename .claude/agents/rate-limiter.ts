...
class AgentRateLimiter {
  ...
  requiresApproval(changes: { files: number; lines: number; complexity?: 'low' | 'medium' | 'high' }): boolean {
    const reasons = [];
  ...
  activatePanicMode(reason: string) {
    ...
    require('fs').writeFileSync('.claude/agents/PANIC_MODE.json', JSON.stringify({
  ...
  private triggerCooldown(reason: string) {
    ...
    require('fs').writeFileSync('.claude/agents/cooldown.json', JSON.stringify({
      reason,
      endsAt: cooldownEnd,
  ...
  getStatus(): {
    ...
    try {
      const cooldownData = JSON.parse(require('fs').readFileSync('.claude/agents/cooldown.json', 'utf8'));
...
export const canCreatePR = (changes: { files: number; lines: number; complexity?: 'low' | 'medium' | 'high' }) => {
  return !rateLimiter.requiresApproval(changes) && !rateLimiter.isRateLimited();
};

export const recordPRCreation = (files: number, stackDepth: number, author: 'human' | 'beast' = 'beast') => {
...
export const agentStatus = () => {
  ...
  console.log(`📊 Activity: ${status.recentPRs}/5 PRs this hour, max stack: ${status.maxStackDepth}/3`);
